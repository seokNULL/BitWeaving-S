...
echo "rows,bits,threads,avg_time_s,speedup_vs_1thread,verified" > "$OUT"
...
for n in $ROWS_LIST; do
    for b in $BITS_LIST; do
        ...
        for t in $THREADS_LIST; do
            ...
            echo "$n,$b,$t,$avg,$speedup,$verified" >> "$OUT"
            printf "rows=%-9s bits=%-2s threads=%-2s avg=%-10ss speedup=%-5s verified=%s\n" \
                   "$n" "$b" "$t" "$avg" "$speedup" "$verified"
        done
    done
done
...
awk -F, '{printf "%-10s %-6s %-9s %-12s %-19s %s\n", $1, $2, $3, $4, $5, $6}' "$OUT"
