...
echo "rows,bits,threads,avg_time_s,speedup_vs_1thread,avg_pkg_energy_j,avg_dram_energy_j,verified" > "$OUT"
...
for n in $ROWS_LIST; do
    for b in $BITS_LIST; do
        ...
        for t in $THREADS_LIST; do
            ...
            pkg_energy=$(echo "$output" | awk '/Average package energy/ {print $4}')
            dram_energy=$(echo "$output" | awk '/Average DRAM energy/ {print $4}')
            [ -z "$pkg_energy" ] && pkg_energy="n/a"
            [ -z "$dram_energy" ] && dram_energy="n/a"
            ...
            echo "$n,$b,$t,$avg,$speedup,$pkg_energy,$dram_energy,$verified" >> "$OUT"
            printf "rows=%-9s bits=%-2s threads=%-2s avg=%-10ss speedup=%-5s pkg=%-9s dram=%-9s verified=%s\n" \
                   "$n" "$b" "$t" "$avg" "$speedup" "$pkg_energy" "$dram_energy" "$verified"
        done
    done
done
...
awk -F, '{printf "%-10s %-6s %-9s %-12s %-19s %-17s %-18s %s\n", $1, $2, $3, $4, $5, $6, $7, $8}' "$OUT"
